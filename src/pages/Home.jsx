import React, { useReducer, useCallback, useEffect, useState, useRef, useMemo, lazy, Suspense } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { projects, steps } from '../data/homeData';
...
const aboutBackgroundImage = new URL('../assets/slider1/be6b51_87e025bca05249b3970a1e6c2226ed05~mv2.avif', import.meta.url).href;
const ctaBackgroundImage = new URL('../assets/Slider1/photoslide1.avif', import.meta.url).href;
...
// Preload header image on module load with high priority
preloadImage(headerImage, 'high');
...
const StepIcon = React.memo(({ index }) => {
  const icons = [
    <path key="0" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />,
    ...
    <><path key="2a" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path key="2b" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></>
  ];
  return <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">{icons[index]}</svg>;
});
StepIcon.displayName = 'StepIcon';

// Memoized Service Card component for Home page
const HomeServiceCard = React.memo(({ title, description, icon, link, iconType = 'image' }) => (
  <div className="bg-white rounded-3xl p-10 hover:shadow-2xl transition-all duration-500 hover:-translate-y-3 group w-full max-w-sm mx-auto border border-gray-200 hover:border-[#2a2a2a]/20 h-full">
    <div className="flex flex-col items-center text-center space-y-6 h-full">
      <div className="w-24 h-24 flex items-center justify-center bg-gray-100 rounded-2xl group-hover:bg-[#2a2a2a]/5 transition-all duration-300">
        {iconType === 'image' ? (
          <img src={icon} alt={title} className="w-16 h-16 object-contain group-hover:scale-110 transition-transform duration-300" loading="lazy" />
        ) : (
          <div className="text-[#2a2a2a] group-hover:scale-110 transition-transform duration-300">{icon}</div>
        )}
      </div>
      <h4 className="font-custom font-bold text-2xl text-[#2a2a2a] leading-tight min-h-[3.5rem] flex items-center">{title}</h4>
      <div className="w-16 h-0.5 bg-gradient-to-r from-transparent via-[#2a2a2a]/30 to-transparent"></div>
      <p className="text-gray-600 text-[16px] leading-relaxed font-wix-madefor flex-grow">{description}</p>
      <Link to={link} className="mt-auto px-6 py-2.5 bg-[#2a2a2a] text-white hover:bg-[#5b5b5b] transition-all duration-300 rounded-full font-wix-madefor font-semibold text-sm hover:scale-105 shadow-md">
        Learn More
      </Link>
    </div>
  </div>
...
const Home = () => {
  ...
  const { currentStep, currentProject } = state;
  ...
  const handleSetStep = useCallback((step) => dispatch({ type: 'SET_STEP', payload: step }), []);
  const handleNextStep = useCallback(() => dispatch({ type: 'SET_STEP', payload: currentStep < 2 ? currentStep + 1 : 0 }), [currentStep]);
  const handlePrevStep = useCallback(() => dispatch({ type: 'SET_STEP', payload: currentStep > 0 ? currentStep - 1 : 2 }), [currentStep]);
  
  const handleSetProject = useCallback((project) => dispatch({ type: 'SET_PROJECT', payload: project }), []);
  const handleNextProject = useCallback(() => dispatch({ type: 'SET_PROJECT', payload: currentProject < projects.length - 1 ? currentProject + 1 : 0 }), [currentProject]);
  const handlePrevProject = useCallback(() => dispatch({ type: 'SET_PROJECT', payload: currentProject > 0 ? currentProject - 1 : projects.length - 1 }), [currentProject]);

  // Memoized services data
  const servicesData = useMemo(() => [
    { title: 'Kitchen Renovations', description: 'Modern, elegant, and functional kitchens designed for your lifestyle.', icon: mixerImage, link: '/services/kitchen-renovations', iconType: 'image' },
    { title: 'Bathroom Remodels', description: 'Luxurious upgrades that turn your bathroom into a personal retreat.', icon: bathtubImage, link: '/services/bathroom-remodels', iconType: 'image' },
    { 
      title: 'Home Remodeling', 
      description: 'Custom solutions to enhance style, flow, and comfort throughout your home.', 
    ...
    { 
      ...
      description: 'Expand your living space with expert craftsmanship and seamless integration.', 
  ...
  // Lazy load Elfsight script with Intersection Observer
  useEffect(() => {
    if (!elfsightRef.current || elfsightLoaded) return;
    ...
    <div className="min-h-screen">
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-8">
          <div className="mb-16">
            ...
            {/* Service Cards - Using memoized component */}
          ...
          <div className="relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-0 h-auto md:h-[600px]">
                  ...
                  <div className="bg-primary text-white p-10 md:p-12 flex flex-col justify-center min-h-[400px] md:min-h-0">
                    <div key={currentProject} className="animate-fade-in space-y-6">
                      ...
                      <button onClick={handleNavigateContact} className="px-8 py-3 border-2 border-white text-white hover:bg-white hover:text-primary transition-all duration-300 rounded-lg font-wix-madefor font-semibold w-fit hover:scale-105 shadow-lg">
                        GET A QUOTE
                      </button>
                    </div>
                  </div>
            ...
            <div className="flex justify-center gap-3 mt-8">
              {projects.map((_, index) => (
                <button
                  key={index}
                  onClick={() => handleSetProject(index)}
        ...
        <div className="max-w-7xl mx-auto px-8 relative z-10">
          <div className="max-w-2xl">
            ...
            <div className="space-y-6">
              ...
              <button onClick={handleNavigateContact} className="mt-8 px-10 py-4 bg-primary text-white hover:bg-accent transition-all duration-300 rounded-lg font-wix-madefor font-semibold hover:scale-105 shadow-lg text-[16px]">
                GET A QUOTE
              </button>
            </div>
          </div>
      ...
      <section className="py-16 bg-primary">
        <div className="max-w-4xl mx-auto px-8 text-center">
          ...
          <button onClick={handleNavigateContact} className="px-8 py-3 border-2 border-white text-white hover:bg-white hover:text-primary transition-all duration-300 rounded-lg font-wix-madefor font-semibold hover:scale-105">
            GET A QUOTE
          </button>
        </div>
      </section>
      ...
      <section id="testimonials" className="py-32 bg-white" ref={elfsightRef}>
        <div className="max-w-7xl mx-auto px-8">
          <div className="text-center mb-20">
            {/* Badge */}
          ...
          {/* Elfsight Google Reviews - Only render when near viewport */}
          {elfsightLoaded && (
